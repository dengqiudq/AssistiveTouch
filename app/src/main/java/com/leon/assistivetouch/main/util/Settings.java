...
public class Settings implements Constan{
	...
	private void init (int init_code, int version_code) {
		...
		if (init_code == -1) {
			...
			editor.putString(TOUCH_MAIN_ITEM_DATA_4, String.valueOf(KeyItemInfo.KEY_SETTING));
			editor.putString(TOUCH_MAIN_ITEM_DATA_5, String.valueOf(KeyItemInfo.KEY_HIDE));
			editor.putString(TOUCH_MAIN_ITEM_DATA_6, String.valueOf(KeyItemInfo.KEY_POWER));
	...
	public boolean isEnableAssistiveTouch () {
		return mSpref.getBoolean(ENABLE_ASSISTIVE_KEY, true);
	}
	...
	public boolean isEnableBootStart () {
		return mSpref.getBoolean(ENABLE_BOOT_START_KEY, true);
	}

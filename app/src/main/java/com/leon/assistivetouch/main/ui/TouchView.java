...
public class TouchView {
	...
	private void createTouchMainView() {
		...
		mTouchMainView.setOnTouchListener(mOnTouchClickListener);
	 ...
	 * 触发按键事件处理
	 */
	private void onKeyItemInfoEvent(KeyItemInfo info, Context context) {
		switch (info.getType()) {
			...
			case KeyItemInfo.TYPE_KEY:
				if (info.getData().equals(String.valueOf(KeyItemInfo.KEY_HIDE))) {
					...
					KeyItemInfo.doKeyEvent(info.getData(), context);
				}
				break;
	...
	private TouchPanelView.OnKeyClickListener mOnKeyClickListener =
			new TouchPanelView.OnKeyClickListener() {
				...
				public void onClick(int postion, KeyItemInfo info) {
					onKeyItemInfoEvent(info, mContext);
	...
	//点击外部显示TouchDotView
	private TouchPanelView.OnTouchListener mOnTouchClickListener =
			new TouchPanelView.OnTouchListener() {
